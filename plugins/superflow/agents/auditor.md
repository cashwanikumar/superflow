...
These are **scan hints** — where to find the evidence for each section of the
template (see "Output structure" below), not the section list itself. The
template is the authority on which sections exist; if it gains one, look for its
evidence the same way. Skip any that don't apply.
...
4. **Write the rulebook.** Load the `superflow:codebase-rulebook` skill and fill in its template — it is the single source of truth for the file's structure.
...
You do **not** carry the rulebook's structure in your head. The canonical
`CODEBASE_RULEBOOK.md` template — every section, in order, with the shape each
one's content must take — lives in the **`superflow:codebase-rulebook`** skill.

**Load that skill before you write, and fill in its template verbatim.** Omit
sections that don't apply to this repo; never invent a section it doesn't define,
and never write a section from memory. If the template and anything else you
have read disagree, the template wins.
