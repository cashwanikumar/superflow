...
- **Personas 9 → 5:** `sherlock`, `architect`, `designer`, `codezilla`, `bughunter`. Same model, different adjectives was not worth four extra prompts. `unit-tester` folded into `codezilla` (TDD skill already owned the method); `a11y-hunter` is now an accessibility section of `bughunter` (WCAG 2.0 AA, same bar); `auditor` folded into the `codebase-rulebook` skill (sherlock scans, the lead writes); `bossbaby`'s product framing moved into `architect`'s Plan stage, and its council vote is a plain lensed agent.
...
- **Dropped external vendor CLIs from `/superflow:council`.** The secret-sanitization and sandboxing were prompt text, not code, and the README read like enforcement. Council is four in-session lenses now.
